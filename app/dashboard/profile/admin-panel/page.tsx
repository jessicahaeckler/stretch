...
import ToggleEmailVerifiedInput from "./_components/toggle-email-verify-input";
            ...
            {users.map((user) => (
              <tr
                ...
                className={cn("divide-x", {
                  "bg-primary/5": user.role === USER_ROLES.ADMIN,
                })}
              >
  ...
  return (
    <Button size="lg">
      <Link className="flex" href="/dashboard/profile">
        <ArrowLeftSquareIcon className="m-auto mr-2" />
        Profile
      </Link>
    </Button>
  );
